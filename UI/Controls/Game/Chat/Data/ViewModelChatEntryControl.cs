    ...
    public class ViewModelChatEntryControl : BaseViewModel
    {
        ...
        private void UpdateText(InlineCollection inlines)
        {
            ...
            if (!isServiceMessage)
            {
                ...
                if (DevelopersListHelper.IsDeveloper(displayedName))
                {
                    displayedName = ChatNamePrefix_Developer + NoBreakSpace + displayedName;
                }

                if (VeryLazyGuysList.IsLazyGuy(displayedName))
                {
                    displayedName = VeryLazyGuysList.Tag(displayedName) + NoBreakSpace + displayedName;
                }
                inlines.Add(new Run(displayedName) { FontWeight = FontWeights.SemiBold });
            }
